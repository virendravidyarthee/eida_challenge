...
const totalProductionTodayPage: NextPage = (props: {
    recordList?: [Record],
    error?: Error
}) => {
    if (!props.error) {
        if (props.recordList?.length ?? 0 > 0) {
            ...
            return (
                <>
                    <Head>
                        <title>Total daily production</title>
                    </Head>
                    ...
                    <Card shadow={"md"} radius={"md"}>
                        <Text weight={700}>Total daily production</Text>
                        <Text weight={500}>{latestRecord.daily_prod_KWH} kwh</Text>
                        <br />
                        <LineChart data={props.recordList!} xAxisKey="inverter_update_time" yAxisKey="daily_prod_KWH"></LineChart>
                    </Card>
                </>
            )
        } else {
...
export default totalProductionTodayPage