...
const batteryStatusPage: NextPage = (props: {
    recordList?: [Record],
    error?: Error
}) => {
    if (!props.error) {
        if (props.recordList?.length ?? 0 > 0) {
            ...
            return (
                <div>
                    ...
                    <Text><Text component="span" weight={"bold"}>Battery percentage: </Text>{latestRecord.battery_prc}%</Text>
                    <Text><Text component="span" weight={"bold"}>Battery status: </Text>{latestRecord.battery_power ? 'Charging' : 'Discharging'}</Text>
                    <br />
                    <Center>
                        <RingProgress
                            roundCaps
                            sections={[{ value: Number(latestRecord.battery_prc), color: 'blue' }]}
                            label={
                                <Text color="blue" weight={700} align="center" size="xl">
                                    {latestRecord.battery_prc}%
                                </Text>
                            }
                        />
                    </Center>
