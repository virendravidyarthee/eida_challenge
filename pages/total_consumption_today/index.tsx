...
const totalConsumptionTodayPage: NextPage = (props: {
    recordList?: [Record],
    error?: Error
}) => {
    if (!props.error) {
        if (props.recordList?.length ?? 0 > 0) {
            ...
            return (
                <>
                    <Head>
                        <title>Total daily consumption</title>
                    </Head>
                    ...
                    <Card shadow={"md"} radius={"md"}>
                        <Text weight={700}>Total daily consumption</Text>
                        <Text weight={500}>{latestRecord.consumption_daily} kwh</Text>
                        <br />
                        <LineChart data={props.recordList!} xAxisKey="inverter_update_time" yAxisKey="consumption_daily"></LineChart>
                    </Card>
                </>
            )
        } else {
...
export default totalConsumptionTodayPage